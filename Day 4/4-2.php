...
echo 'Day 4, problem 2: ' . array_reduce(explode("\n", $input), function ($validPassphrases, $passphrase) {
    $chars = [];
    $containsAnagram = false;
    foreach (explode(' ', $passphrase) as $word) {
        $wordChars = count_chars($word, 1);

        if (in_array($wordChars, $chars)) {
    ...
    return $containsAnagram ? $validPassphrases : ++$validPassphrases;
}, 0);
